...
请确保目标机器已安装以下软件：
1.  **Python 3.9.x+** (用于后端服务)
    #### 注意：务必采用版本偏低的python（如3.9.x,3.10.x）否则将会导致程序无法正确运行！
...
2.  **安装依赖**：
    ...
    ```
3.  **配置数字人密钥**：
    请从[魔珐星云官网](https://www.xingyun3d.com/) 自行注册账户，并在控制台构建自己的数字人形象，将生成后的APP ID,APP Secrect妥善保存（务必不要随意泄露此信息，该信息与数字人形象的计费有关！）
